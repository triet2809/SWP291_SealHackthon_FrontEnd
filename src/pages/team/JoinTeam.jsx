...
const JoinTeam = () => {
  ...
  const handleJoinViaCode = async (e) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to join team. Check the invite code.');
    } finally {
      setJoining(false);
    }
  };

  // NOTE: no BE endpoint for public-team discovery / request-to-join; browse tab stays mock.
  const publicTeams = [
              ...
              <Card.Body className="p-5 text-center">
                ...
                <Form onSubmit={handleJoinViaCode} className="mx-auto" style={{ maxWidth: '350px' }}>
                  ...
                  <Button variant="primary" type="submit" size="lg" className="w-100 d-flex align-items-center justify-content-center gap-2" disabled={joining || !!success}>
                    {joining ? <Spinner animation="border" size="sm" /> : <>Join Team <ArrowRight size={20} /></>}
                  </Button>
                </Form>
              </Card.Body>
            </Card>
